package def.express.express_serve_static;

import def.node.fs.Stats;
import def.node.http.ServerResponse;
import jsweet.util.union.Union3;

@jsweet.lang.Interface
public abstract class ServeStaticOptions extends def.js.Object {
	/**
	 * Set how "dotfiles" are treated when encountered. A dotfile is a file or
	 * directory that begins with a dot ("."). Note this check is done on the
	 * path itself without checking if the path actually exists on the disk. If
	 * root is specified, only the dotfiles above the root are checked (i.e. the
	 * root itself can be within a dotfile when when set to "deny"). The default
	 * value is 'ignore'. 'allow' No special treatment for dotfiles 'deny' Send
	 * a 403 for any request for a dotfile 'ignore' Pretend like the dotfile
	 * does not exist and call next()
	 */
	@jsweet.lang.Optional
	public String dotfiles;
	/**
	 * Enable or disable etag generation, defaults to true.
	 */
	@jsweet.lang.Optional
	public Boolean etag;
	/**
	 * Set file extension fallbacks. When set, if a file is not found, the given
	 * extensions will be added to the file name and search for. The first that
	 * exists will be served. Example: ['html', 'htm']. The default value is
	 * false.
	 */
	@jsweet.lang.Optional
	public String[] extensions;
	/**
	 * Let client errors fall-through as unhandled requests, otherwise forward a
	 * client error. The default value is false.
	 */
	@jsweet.lang.Optional
	public Boolean fallthrough;
	/**
	 * By default this module will send "index.html" files in response to a
	 * request on a directory. To disable this set false or to supply a new
	 * index pass a string or an array in preferred order.
	 */
	@jsweet.lang.Optional
	public Union3<Boolean, String, String[]> index;
	/**
	 * Enable or disable Last-Modified header, defaults to true. Uses the file
	 * system's last modified value.
	 */
	@jsweet.lang.Optional
	public Boolean lastModified;
	/**
	 * Provide a max-age in milliseconds for http caching, defaults to 0. This
	 * can also be a string accepted by the ms module.
	 */
	@jsweet.lang.Optional
	public jsweet.util.union.Union<Double, String> maxAge;
	/**
	 * Redirect to trailing "/" when the pathname is a dir. Defaults to true.
	 */
	@jsweet.lang.Optional
	public Boolean redirect;
	/**
	 * Function to set custom headers on response. Alterations to the headers
	 * need to occur synchronously. The function is called as fn(res, path,
	 * stat), where the arguments are: res the response object path the file
	 * path that is being sent stat the stat object of the file that is being
	 * sent
	 */
	@jsweet.lang.Optional
	public jsweet.util.function.TriFunction<ServerResponse, String, Stats, Object> setHeaders;
}
